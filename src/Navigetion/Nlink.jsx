...
const Nlink = ({ alert, close }) => {
  const [isVisible, setIsVisible] = useState(alert); // Local state to handle visibility

  useEffect(() => {
    if (alert) {
      // If alert is true, show the component
      setIsVisible(true);
    } else {
      // If alert is false, delay hiding to allow the animation to complete
      const timeout = setTimeout(() => setIsVisible(false), 450); // Match the animation duration
        ...
        <nav
          className={`pt-[1.8rem] md:inline-flex md:absolute md:right-0 md:top-[60%] md:-translate-y-[88%] ${
            alert
              ...
              : "animate__animated animate__fadeOutUp" // Animation for hiding
          } md:block`}
        >
